...
public abstract class Activity(string date, double lengthOfActivity)
{
    ...
    public string Summary()
    {
        ...
        return $"  \u25e6 {_date} {GetActivityType()} ({GetLengthOfActivity().ToString("F1")} min) - Distance: {CalcDistance().ToString("F1")} km, Speed: {CalcSpeed().ToString("F1")} kph, Pace: {CalcPace().ToString("F1")} min per km";
    }
